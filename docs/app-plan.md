...
This milestone is complete. The Rust packet library, HID transport boundary, CLI, dry-run output, steady/off zone controls, and guarded exact-capture replay are covered by deterministic tests. Physical tests verified non-root left/right colors, microphone live/muted colors, every earcup/microphone/all-zone off target, and exact Synchronized replay. Removing the udev rule and logically reconnecting made access fail closed; reinstalling and both logical and manual cable reconnects restored access automatically. The GameDAC 5.1 audio function recovered after each reconnect and remained functional.
...
This milestone is complete for single-color Breathe, Synchronized, Sweep, two-color ColorShift, and one-to-four-color Multi Color Breathe. Generated two-color ColorShift reports reproduce normal and reversed-order Engine fixtures byte-for-byte; a red/blue hardware test traversed purple. A generated red/green/blue Multi Color Breathe displayed that order and faded to black between colors. The builder accepts only whole seconds from 1 through 30 and rejects reverse outside Sweep. Engine's reverse flag is reproduced exactly, although its visible direction could not be distinguished from normal reliably on two alternating zones after the apply flash. Longer ColorShift lists, arbitrary GG marker positions, fractional multicolor speed mapping, and Reflected remain fail-closed.
...
This functional milestone is complete. The current Rust GTK4/libadwaita window presents the verified effects as Solid, Color Flow, and Color Pulse while retaining the protocol's Steady, ColorShift, Multi Color Breathe, and legacy single-color Breathe/Sweep identities in stored data and diagnostics. It saves a versioned profile store under the XDG configuration directory, remembers the last selected profile independently from a master lighting state, and keeps reconnect restoration opt-in. The dedicated UX pass added synchronized native color dialogs and exact hex entry, an ordered animated palette, always-visible independent microphone controls, explicit profile creation and confirmed deletion, a built-in symbol picker, adaptive narrow controls, standard keyboard shortcuts, accessible names, event toasts, persistent corruption banners, an original application icon, and a master switch that applies verified all-zone black without discarding the selected profile. Repeated instructional copy was consolidated into an accessible header-level How to use dialog with an F1 shortcut; only essential per-control constraints remain inline, and switches are centered at their natural height across desktop themes. Storage mutations reload the latest validated file under a cross-process advisory lock and use a private, same-directory, synchronized atomic replacement; malformed data is never automatically replaced. Hardware acceptance saved and restored a profile across a full process restart, applied three- and four-color Breathe palettes, detected a manually re-enumerated hidraw path, waited through the GameDAC firmware readiness window, restored both earcups, and reported transient device access separately from a persistent permission error. PipeWire retained the GameDAC Game 5.1 default sink at 60 percent and audio returned after reconnect. A separate research task will test whether passive 64-byte HID input can safely replace the conservative four-second readiness delay.
