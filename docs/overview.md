...
`gamedacctl` is the native Rust controller. Its pure protocol layer constructs zero-filled steady reports, computes exact zone masks, validates captured reports, and is isolated from the HID transport. It selects only `1038:1280` interface 0. Tests cover all bytes in generated steady reports and hash every byte of the known Sweep, Synchronized, and five-second Breathe capture fixtures. Physical acceptance verified its static, all three off-target, microphone-state, and exact-replay paths without disrupting audio.
