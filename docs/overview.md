...
Captured but not yet fully implemented:

- Off.
- Breathe at three speeds.
- ColorShift or multicolor-breathe coefficient sets.
- Independent per-zone animation coefficients and direction variants.

## Current utility

`scripts/gamedac-rgb` uses the installed HIDAPI library through Python `ctypes`. It finds vendor `1038`, product `1280`, interface 0, sends the observed 1,024-byte feature reports, and follows them with observed 64-byte apply/save reports.
...
- The exact mathematical meaning of animation coefficient records after byte 11.
- Whether arbitrary multicolor gradients can be generated safely or should initially be exposed as captured presets.
- Why requested right-ear color `#FF3700` appeared as `FF3C00` in a captured header; Engine quantization or UI state is plausible but unverified.
