...
The accepted public project, package, and binary name is `gamedacctl`; the GitHub repository is `AndreasDellrud/gamedacctl`. The Unix-style `ctl` suffix describes a controller without presenting it as an official SteelSeries product. A 2026-09-05 exact GitHub repository-name search returned only this project, and crates.io returned no package with the name. The repository description says “Unofficial Linux lighting control,” and the README carries the explicit non-affiliation and compatibility-only disclaimer. The application ships an independently drawn dial-and-RGB-arc icon plus a monochrome symbolic variant. It uses no SteelSeries logo, headset silhouette, GG artwork, or copied trade dress; future artwork must preserve that boundary.

The project is dual-licensed under `MIT OR Apache-2.0`. This applies to the independently written source, documentation, and packaging; it does not grant rights in SteelSeries names, software, firmware, or other third-party material. The Apache contributor patent grant covers only patent claims licensable by a contributor and is not a license from SteelSeries.
